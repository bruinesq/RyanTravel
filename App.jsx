import { useState, useEffect, useCallback, useRef } from 'react'
import { supabase } from './supabase.js'
import jsPDF from 'jspdf'

// ───────────────────────────────────────────────────────────────
// CONSTANTS
// ───────────────────────────────────────────────────────────────
const CATEGORIES = ['Meals', 'Flight', 'Gas', 'Lodging', 'Activities', 'Other']
const CAT_CLASS = {
  Meals: 'cat-meals', Flight: 'cat-flight', Gas: 'cat-gas',
  Lodging: 'cat-lodging', Activities: 'cat-activities', Other: 'cat-other',
}

function fmtMoney(n) { return '$' + Number(n || 0).toFixed(2) }
function fmtDate(str) {
  if (!str) return ''
  const [y, m, d] = str.split('-')
  return `${m}/${d}/${y}`
}
function todayStr() { return new Date().toISOString().split('T')[0] }

// ───────────────────────────────────────────────────────────────
// INLINE CALCULATOR (telephone-pad style, renders below field)
// Keys: 1-2-3 / 4-5-6 / 7-8-9 / C-0-⌫ / ops row / = row
// ───────────────────────────────────────────────────────────────
function InlineCalc({ initial, onConfirm }) {
  const [display, setDisplay] = useState(initial ? String(initial) : '0')
  const [expr, setExpr]       = useState(initial ? String(initial) : '')
  const [fresh, setFresh]     = useState(!initial)

  function press(val) {
    if (val === 'C')  { setDisplay('0'); setExpr(''); setFresh(true); return }
    if (val === '⌫') {
      const s = display.length <= 1 ? '0' : display.slice(0, -1)
      setDisplay(s); setExpr(s); setFresh(false); return
    }
    if (val === '=') {
      try {
        const r = parseFloat(Function('"use strict";return(' + expr + ')')().toFixed(2)).toString()
        setDisplay(r); setExpr(r); setFresh(true)
      } catch { setDisplay('ERR') }
      return
    }
    const isOp = ['+', '-', '×', '÷', '.'].includes(val)
    const actual = val === '×' ? '*' : val === '÷' ? '/' : val
    if (fresh && !isOp) {
      setDisplay(val); setExpr(actual); setFresh(false)
    } else {
      const next = (expr === '0' && !isOp) ? actual : expr + actual
      setDisplay(next.replace(/\*/g, '×').replace(/\//g, '÷'))
      setExpr(next); setFresh(false)
    }
  }

  // Telephone pad: 1,2,3 / 4,5,6 / 7,8,9 / C,0,⌫  +  ops column
  const rows = [
    ['1','2','3'],
    ['4','5','6'],
    ['7','8','9'],
    ['C','0','⌫'],
  ]
  const ops = ['+','-','×','÷']

  return (
    <div className="inline-calc">
      <div className="calc-display-inline">{display}</div>
      <div className="calc-body">
        <div className="calc-main-grid">
          {rows.map((row, ri) => (
            <div key={ri} className="calc-row">
              {row.map(k => (
                <button
                  key={k}
                  className={`calc-btn${k==='C'?' clear':k==='⌫'?' back':''}`}
                  onClick={() => press(k)}
                >{k}</button>
              ))}
            </div>
          ))}
          <div className="calc-row">
            <button className="calc-btn dot" onClick={() => press('.')}>.</button>
            <button className="calc-btn equals" onClick={() => press('=')}>=</button>
            <button
              className="calc-btn use-btn"
              onClick={() => {
                const v = display === 'ERR' ? 0 : parseFloat(display) || 0
                onConfirm(v)
              }}
            >Use</button>
          </div>
        </div>
        <div className="calc-ops-col">
          {ops.map(k => (
            <button key={k} className="calc-btn op" onClick={() => press(k)}>{k}</button>
          ))}
        </div>
      </div>
    </div>
  )
}

// ───────────────────────────────────────────────────────────────
// INLINE DATE PICKER (renders below field, auto-confirms on change)
// ───────────────────────────────────────────────────────────────
function InlineDatePicker({ initial, onConfirm }) {
  const ref = useRef()
  useEffect(() => {
    // Auto-open the native date picker on mount
    if (ref.current) {
      try { ref.current.showPicker() } catch {}
    }
  }, [])
  return (
    <div className="inline-date-picker">
      <input
        ref={ref}
        type="date"
        className="date-input-large"
        defaultValue={initial || todayStr()}
        max={todayStr()}
        onChange={e => { if (e.target.value) onConfirm(e.target.value) }}
      />
    </div>
  )
}

// ───────────────────────────────────────────────────────────────
// PARTICIPANTS INLINE PANEL — 3 per row, unchecked by default
// ───────────────────────────────────────────────────────────────
function ParticipantsPanel({ members, selected, onChange }) {
  function toggle(name) {
    const s = new Set(selected)
    s.has(name) ? s.delete(name) : s.add(name)
    onChange(Array.from(s))
  }
  // Group into rows of 3, preserving insertion order
  const rows = []
  for (let i = 0; i < members.length; i += 3) rows.push(members.slice(i, i + 3))

  return (
    <div className="participants-panel">
      {rows.map((row, ri) => (
        <div key={ri} className="participant-row">
          {row.map(name => {
            const on = selected.includes(name)
            return (
              <button
                key={name}
                className={`participant-chip${on ? ' checked' : ''}`}
                onClick={() => toggle(name)}
              >
                <span className={`pchip-box${on ? ' on' : ''}`}>{on ? '✓' : ''}</span>
                <span className="pchip-name">{name}</span>
              </button>
            )
          })}
        </div>
      ))}
      <div className="participant-count">{selected.length} of {members.length} selected</div>
    </div>
  )
}

// ───────────────────────────────────────────────────────────────
// TRIP FORM MODAL  — now includes trip_date field
// ───────────────────────────────────────────────────────────────
function TripFormModal({ onSave, onClose }) {
  const [tripName, setTripName]   = useState('')
  const [tripDate, setTripDate]   = useState(todayStr())
  const [members,  setMembers]    = useState(['', ''])
  const [saving,   setSaving]     = useState(false)
  const [error,    setError]      = useState('')

  function addMember()        { setMembers(p => [...p, '']) }
  function removeMember(i)    { setMembers(p => p.filter((_, j) => j !== i)) }
  function setMember(i, val)  { setMembers(p => p.map((m, j) => j === i ? val : m)) }

  async function save() {
    const name = tripName.trim()
    if (!name) { setError('Trip name is required.'); return }
    if (!tripDate) { setError('Trip date is required.'); return }
    const clean = members.map(m => m.trim()).filter(Boolean)
    if (clean.length < 1) { setError('Add at least one member.'); return }
    setSaving(true)
    try {
      const { data: trip, error: te } = await supabase
        .from('trips').insert({ name, trip_date: tripDate }).select().single()
      if (te) throw te
      const { error: me } = await supabase.from('trip_members')
        .insert(clean.map(m_name => ({ trip_id: trip.id, name: m_name })))
      if (me) throw me
      onSave(trip.id)
    } catch (e) {
      setError(e.message || 'Failed to create trip.')
      setSaving(false)
    }
  }

  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-sheet" onClick={e => e.stopPropagation()}>
        <div className="modal-handle" />
        <div className="modal-title">New Trip</div>
        {error && <div className="error-banner">{error}</div>}
        <div className="trip-form-field">
          <label>Trip Name</label>
          <input type="text" placeholder="e.g. Hawaii 2026" value={tripName}
            onChange={e => setTripName(e.target.value)} />
        </div>
        <div className="trip-form-field">
          <label>Trip Date</label>
          <input type="date" value={tripDate} max={todayStr()}
            onChange={e => setTripDate(e.target.value)} />
        </div>
        <div className="trip-form-field">
          <label>Members</label>
          {members.map((m, i) => (
            <div key={i} className="member-input-row">
              <input type="text" placeholder={`Member ${i + 1}`} value={m}
                onChange={e => setMember(i, e.target.value)} />
              {members.length > 1 && (
                <button className="remove-member-btn" onClick={() => removeMember(i)}>✕</button>
              )}
            </div>
          ))}
          <button className="add-member-btn" onClick={addMember}>+ Add Member</button>
        </div>
        <button className="save-trip-btn" onClick={save} disabled={saving}>
          {saving ? 'Creating...' : 'Create Trip'}
        </button>
        <button className="modal-cancel-btn" onClick={onClose}>Cancel</button>
      </div>
    </div>
  )
}

// ───────────────────────────────────────────────────────────────
// EDIT MEMBERS MODAL
// ───────────────────────────────────────────────────────────────
function EditMembersModal({ tripId, members, onSave, onClose }) {
  const [names,  setNames]  = useState(members.map(m => m.name))
  const [saving, setSaving] = useState(false)
  const [error,  setError]  = useState('')

  function addMember()       { setNames(p => [...p, '']) }
  function removeMember(i)   { setNames(p => p.filter((_, j) => j !== i)) }
  function setName(i, val)   { setNames(p => p.map((n, j) => j === i ? val : n)) }

  async function save() {
    const clean = names.map(n => n.trim()).filter(Boolean)
    if (clean.length < 1) { setError('At least one member required.'); return }
    setSaving(true)
    try {
      await supabase.from('trip_members').delete().eq('trip_id', tripId)
      const { error: e } = await supabase.from('trip_members')
        .insert(clean.map(n => ({ trip_id: tripId, name: n })))
      if (e) throw e
      onSave()
    } catch (e) {
      setError(e.message || 'Failed to save members.')
      setSaving(false)
    }
  }

  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-sheet" onClick={e => e.stopPropagation()}>
        <div className="modal-handle" />
        <div className="modal-title">Edit Members</div>
        {error && <div className="error-banner">{error}</div>}
        {names.map((n, i) => (
          <div key={i} className="member-input-row">
            <input type="text" placeholder={`Member ${i + 1}`} value={n}
              onChange={e => setName(i, e.target.value)} />
            {names.length > 1 && (
              <button className="remove-member-btn" onClick={() => removeMember(i)}>✕</button>
            )}
          </div>
        ))}
        <button className="add-member-btn" onClick={addMember}>+ Add Member</button>
        <button className="save-trip-btn" onClick={save} disabled={saving}>
          {saving ? 'Saving...' : 'Save Members'}
        </button>
        <button className="modal-cancel-btn" onClick={onClose}>Cancel</button>
      </div>
    </div>
  )
}

// ───────────────────────────────────────────────────────────────
// EXPENSE FORM SCREEN
// Active field drives which inline panel is shown below it.
// ───────────────────────────────────────────────────────────────
function ExpenseFormScreen({ trip, members, expense, onSave, onCancel }) {
  const [date,         setDate]         = useState(expense?.expense_date || todayStr())
  const [amount,       setAmount]       = useState(expense?.amount?.toString() || '')
  const [description,  setDescription]  = useState(expense?.description || '')
  const [category,     setCategory]     = useState(expense?.category || 'Meals')
  const [paidBy,       setPaidBy]       = useState(
    expense?.paid_by || members[0]?.name || ''
  )
  // Participants: empty by default for new expense (requirement #5)
  const [participants, setParticipants] = useState(
    expense?.expense_participants?.map(p => p.member_name) || []
  )
  // activePanel: 'date' | 'calc' | 'participants' | null
  const [activePanel, setActivePanel] = useState(null)
  const [saving,  setSaving]  = useState(false)
  const [error,   setError]   = useState('')

  function togglePanel(name) {
    setActivePanel(p => p === name ? null : name)
  }

  async function save() {
    if (!amount || parseFloat(amount) <= 0) { setError('Amount must be greater than 0.'); return }
    if (!description.trim())                { setError('Description is required.'); return }
    if (!paidBy)                            { setError('Select who paid.'); return }
    if (participants.length === 0)          { setError('Select at least one participant.'); return }
    setSaving(true)
    try {
      let expenseId = expense?.id
      if (expenseId) {
        const { error: ue } = await supabase.from('expenses').update({
          expense_date: date, amount: parseFloat(amount),
          description: description.trim(), category, paid_by: paidBy,
        }).eq('id', expenseId)
        if (ue) throw ue
        await supabase.from('expense_participants').delete().eq('expense_id', expenseId)
      } else {
        const { data: newExp, error: ie } = await supabase.from('expenses')
          .insert({ trip_id: trip.id, expense_date: date, amount: parseFloat(amount),
            description: description.trim(), category, paid_by: paidBy })
          .select().single()
        if (ie) throw ie
        expenseId = newExp.id
      }
      const { error: pe } = await supabase.from('expense_participants')
        .insert(participants.map(m_name => ({ expense_id: expenseId, member_name: m_name })))
      if (pe) throw pe
      onSave()
    } catch (e) {
      setError(e.message || 'Failed to save expense.')
      setSaving(false)
    }
  }

  const memberNames = members.map(m => m.name) // preserves insertion order

  return (
    <div className="screen-content">
      <div className="form-screen">
        <h2>{expense ? 'Edit Expense' : 'New Expense'}</h2>
        {error && <div className="error-banner">{error}</div>}

        {/* Row 1: Date | Amount */}
        <div className="form-row">
          <div className="form-field">
            <label>Date</label>
            <div
              className={`tappable-field${activePanel==='date'?' active-field':''}`}
              onClick={() => togglePanel('date')}
            >
              <span>{fmtDate(date)}</span>
              <span className="tappable-icon">📅</span>
            </div>
          </div>
          <div className="form-field">
            <label>Amount</label>
            <div
              className={`tappable-field${!amount?' placeholder':''}${activePanel==='calc'?' active-field':''}`}
              onClick={() => togglePanel('calc')}
            >
              <span>{amount ? fmtMoney(amount) : '$0.00'}</span>
              <span className="tappable-icon">🧮</span>
            </div>
          </div>
        </div>

        {/* Inline date picker — appears immediately below Row 1 */}
        {activePanel === 'date' && (
          <InlineDatePicker
            initial={date}
            onConfirm={v => { setDate(v); setActivePanel(null) }}
          />
        )}

        {/* Inline calculator — appears immediately below Row 1 */}
        {activePanel === 'calc' && (
          <InlineCalc
            initial={amount}
            onConfirm={v => { setAmount(v.toString()); setActivePanel(null) }}
          />
        )}

        {/* Row 2: Description */}
        <div className="form-field-full">
          <div className="form-field">
            <label>Description</label>
            <input type="text" placeholder="What was purchased?"
              value={description}
              onChange={e => setDescription(e.target.value)}
              onFocus={() => setActivePanel(null)}
            />
          </div>
        </div>

        {/* Row 3: Category | Paid By */}
        <div className="form-row">
          <div className="form-field">
            <label>Category</label>
            <select value={category} onChange={e => setCategory(e.target.value)}
              onFocus={() => setActivePanel(null)}>
              {CATEGORIES.map(c => <option key={c}>{c}</option>)}
            </select>
          </div>
          <div className="form-field">
            <label>Paid By</label>
            <select value={paidBy} onChange={e => setPaidBy(e.target.value)}
              onFocus={() => setActivePanel(null)}>
              {memberNames.map(n => <option key={n}>{n}</option>)}
            </select>
          </div>
        </div>

        {/* Row 4: Participants toggle */}
        <button
          className={`participants-btn${activePanel==='participants'?' active-field':''}`}
          onClick={() => togglePanel('participants')}
        >
          <span>
            Participants:{' '}
            {participants.length === 0
              ? 'None selected'
              : participants.length === memberNames.length
              ? 'All members'
              : participants.join(', ')}
          </span>
          <span>{activePanel==='participants' ? '▲' : '▼'}</span>
        </button>

        {/* Inline participants panel */}
        {activePanel === 'participants' && (
          <ParticipantsPanel
            members={memberNames}
            selected={participants}
            onChange={setParticipants}
          />
        )}

        <div className="form-actions">
          <button className="btn-primary" onClick={save} disabled={saving}>
            {saving ? 'Saving...' : expense ? 'Update' : 'Add Expense'}
          </button>
          <button className="btn-secondary" onClick={onCancel}>Cancel</button>
        </div>
      </div>
    </div>
  )
}

// ───────────────────────────────────────────────────────────────
// EXPENSES LIST SCREEN
// ───────────────────────────────────────────────────────────────
function ExpensesScreen({ expenses, onEdit, onDelete }) {
  const [sortField, setSortField] = useState('most_recent')
  const [sortDir,   setSortDir]   = useState('desc')

  function cycleSort(field) {
    if (sortField === field) setSortDir(d => d === 'asc' ? 'desc' : 'asc')
    else { setSortField(field); setSortDir('desc') }
  }

  const sorted = [...expenses].sort((a, b) => {
    let cmp = 0
    if (sortField === 'most_recent' || sortField === 'by_date')
      cmp = a.expense_date < b.expense_date ? -1 : a.expense_date > b.expense_date ? 1 : 0
    else if (sortField === 'by_payor')
      cmp = (a.paid_by || '').localeCompare(b.paid_by || '')
    if (sortField === 'most_recent') cmp = -cmp
    return sortDir === 'asc' ? cmp : -cmp
  })

  return (
    <div className="screen-content">
      <div className="expenses-screen">
        <div className="sort-bar">
          <div className="sort-chevrons">
            <button className={sortDir==='asc'?'active':''} onClick={() => setSortDir('asc')}>▲</button>
            <button className={sortDir==='desc'?'active':''} onClick={() => setSortDir('desc')}>▼</button>
          </div>
          <div className="sort-options">
            {[{key:'most_recent',label:'Most Recent'},{key:'by_date',label:'By Date'},{key:'by_payor',label:'By Payor'}]
              .map(opt => (
                <button key={opt.key}
                  className={`sort-option-btn${sortField===opt.key?' active':''}`}
                  onClick={() => cycleSort(opt.key)}>
                  {opt.label}
                </button>
              ))}
          </div>
        </div>
        {sorted.length === 0 ? (
          <div className="no-expenses">
            <p>No expenses yet.</p>
            <p>Go to Home and tap "+ Enter Expense".</p>
          </div>
        ) : (
          <div className="expense-list">
            {sorted.map(exp => (
              <div key={exp.id} className={`expense-item ${CAT_CLASS[exp.category]||'cat-other'}`}>
                <div className="expense-info">
                  <div className="expense-top">
                    <span className="expense-category">{exp.category}</span>
                    <span>•</span>
                    <span className="expense-desc">{exp.description}</span>
                  </div>
                  <div className="expense-meta">
                    {fmtDate(exp.expense_date)} · Paid by {exp.paid_by}
                    {exp.expense_participants?.length > 0 && (
                      <span> · {exp.expense_participants.map(p => p.member_name).join(', ')}</span>
                    )}
                  </div>
                </div>
                <div className="expense-amount">{fmtMoney(exp.amount)}</div>
                <div className="expense-actions">
                  <button onClick={() => onEdit(exp)} title="Edit">✎</button>
                  <button className="delete-btn" onClick={() => onDelete(exp.id)} title="Delete">🗑</button>
                </div>
              </div>
            ))}
          </div>
        )}
      </div>
    </div>
  )
}

// ───────────────────────────────────────────────────────────────
// SUMMARY SCREEN
// ───────────────────────────────────────────────────────────────
function SummaryScreen({ trip, expenses, members }) {
  const [view, setView] = useState('category')
  const total = expenses.reduce((s, e) => s + parseFloat(e.amount || 0), 0)

  const byCategory = CATEGORIES.reduce((acc, cat) => {
    const sum = expenses.filter(e => e.category === cat)
      .reduce((s, e) => s + parseFloat(e.amount || 0), 0)
    if (sum > 0) acc[cat] = sum
    return acc
  }, {})

  const perPerson = {}
  members.forEach(m => { perPerson[m.name] = 0 })
  expenses.forEach(exp => {
    const parts = exp.expense_participants?.map(p => p.member_name) || []
    if (!parts.length) return
    const share = parseFloat(exp.amount || 0) / parts.length
    parts.forEach(n => { if (perPerson[n] !== undefined) perPerson[n] += share })
  })

  const paid = {}
  members.forEach(m => { paid[m.name] = 0 })
  expenses.forEach(exp => {
    if (paid[exp.paid_by] !== undefined) paid[exp.paid_by] += parseFloat(exp.amount || 0)
  })
  const balance = {}
  members.forEach(m => { balance[m.name] = (paid[m.name]||0) - (perPerson[m.name]||0) })

  const cArr = Object.entries(balance).filter(([,v])=>v>0).sort((a,b)=>b[1]-a[1]).map(([n,v])=>({n,v}))
  const dArr = Object.entries(balance).filter(([,v])=>v<0).sort((a,b)=>a[1]-b[1]).map(([n,v])=>({n,v:-v}))
  const settlements = []
  let ci=0, di=0
  while (ci<cArr.length && di<dArr.length) {
    const amt = Math.min(cArr[ci].v, dArr[di].v)
    if (amt>0.01) settlements.push({from:dArr[di].n, to:cArr[ci].n, amount:amt})
    cArr[ci].v -= amt; dArr[di].v -= amt
    if (cArr[ci].v<0.01) ci++
    if (dArr[di].v<0.01) di++
  }

  function exportPDF() {
    const doc = new jsPDF({unit:'pt',format:'letter'})
    let y=40; const lm=40, pw=532
    doc.setFontSize(22); doc.setFont('helvetica','bold')
    doc.text(trip.name+' — Trip Summary', lm, y); y+=30
    doc.setFontSize(11); doc.setFont('helvetica','normal')
    doc.text('Generated: '+new Date().toLocaleDateString(), lm, y); y+=20
    doc.setFontSize(14); doc.setFont('helvetica','bold')
    doc.text('Total Trip Cost: '+fmtMoney(total), lm, y); y+=30

    const section = (title, rows) => {
      doc.setFontSize(13); doc.setFont('helvetica','bold')
      doc.text(title, lm, y); y+=6
      doc.setDrawColor(180,180,180); doc.line(lm,y,lm+pw,y); y+=14
      doc.setFontSize(11); doc.setFont('helvetica','normal')
      rows.forEach(([l,r]) => {
        if (y>720) { doc.addPage(); y=40 }
        doc.text(l, lm, y); doc.text(r, lm+pw, y, {align:'right'}); y+=16
      })
      y+=10
    }

    section('By Category', Object.entries(byCategory).map(([c,a])=>[c,fmtMoney(a)]))
    section('Per Person', Object.entries(perPerson).map(([n,a])=>[n,fmtMoney(a)]))
    section('Settlement', settlements.length
      ? settlements.map(s=>[`${s.from} → ${s.to}`,fmtMoney(s.amount)])
      : [['No settlements needed.','']])
    section('All Expenses',
      [...expenses].sort((a,b)=>a.expense_date<b.expense_date?-1:1)
        .map(e=>[`${fmtDate(e.expense_date)} · ${e.category} · ${e.description} · Paid by ${e.paid_by}`,fmtMoney(e.amount)])
    )
    doc.save(trip.name.replace(/\s+/g,'_')+'_summary.pdf')
  }

  return (
    <div className="screen-content">
      <div className="summary-screen">
        <div className="summary-header">
          <h2>Summary</h2>
          <button className="pdf-export-btn" onClick={exportPDF}>Export PDF</button>
        </div>
        <div className="total-cost-display">
          <div className="total-label">Total Trip Cost</div>
          <div className="total-amount">{fmtMoney(total)}</div>
        </div>
        <div className="toggle-btns">
          {[['category','By Category'],['person','Per Person'],['settlement','Settlement']].map(([k,l])=>(
            <button key={k} className={`toggle-btn${view===k?' active':''}`} onClick={()=>setView(k)}>{l}</button>
          ))}
        </div>
        {view==='category' && (
          <div className="summary-section">
            <h3>By Category</h3>
            {!Object.keys(byCategory).length
              ? <div className="summary-row"><span className="row-label">No expenses yet.</span></div>
              : Object.entries(byCategory).map(([cat,amt])=>(
                <div key={cat} className="summary-row">
                  <span className="row-label">{cat}</span>
                  <span className="row-value">{fmtMoney(amt)}</span>
                </div>
              ))}
          </div>
        )}
        {view==='person' && (
          <div className="summary-section">
            <h3>Per Person (Share)</h3>
            {Object.entries(perPerson).map(([name,amt])=>(
              <div key={name} className="summary-row">
                <span className="row-label">{name}</span>
                <span className="row-value">{fmtMoney(amt)}</span>
              </div>
            ))}
          </div>
        )}
        {view==='settlement' && (
          <div className="summary-section">
            <h3>Settlement</h3>
            {!settlements.length
              ? <div className="summary-row"><span className="row-label">Everyone is settled up! ✓</span></div>
              : settlements.map((s,i)=>(
                <div key={i} className="summary-row">
                  <span className="row-label">{s.from} → {s.to}</span>
                  <span className="row-value positive">{fmtMoney(s.amount)}</span>
                </div>
              ))}
          </div>
        )}
      </div>
    </div>
  )
}

// ───────────────────────────────────────────────────────────────
// HOME SCREEN — chevron on trip selector
// ───────────────────────────────────────────────────────────────
function HomeScreen({ trips, currentTrip, members, expenses, onSelectTrip, onEnterExpense, onEditMembers }) {
  return (
    <div className="screen-content">
      <div className="home-screen">
        {trips.length > 1 && (
          <div className="trip-selector-wrap">
            <select
              className="past-trips-select"
              value={currentTrip?.id || ''}
              onChange={e => onSelectTrip(e.target.value)}
            >
              {trips.map(t => (
                <option key={t.id} value={t.id}>
                  {t.name}{t.trip_date ? '  (' + fmtDate(t.trip_date) + ')' : ''}
                </option>
              ))}
            </select>
            <span className="trip-selector-chevron">⇅</span>
          </div>
        )}

        {currentTrip ? (
          <div className="trip-card">
            <div className="trip-header">
              <div className="trip-name">{currentTrip.name}</div>
              {currentTrip.trip_date && (
                <div className="trip-date-badge">{fmtDate(currentTrip.trip_date)}</div>
              )}
            </div>
            <div className="trip-stats">
              <div className="stat-box">
                <div className="stat-value">{fmtMoney(expenses.reduce((s,e)=>s+parseFloat(e.amount||0),0))}</div>
                <div className="stat-label">Total Spent</div>
              </div>
              <div className="stat-box">
                <div className="stat-value">{expenses.length}</div>
                <div className="stat-label">Expenses</div>
              </div>
              <div className="stat-box">
                <div className="stat-value">{members.length}</div>
                <div className="stat-label">Members</div>
              </div>
            </div>
            <div className="members-section">
              <div className="members-header">
                <span className="members-title">Members</span>
                <button className="edit-members-btn" onClick={onEditMembers}>✎</button>
              </div>
              <div className="members-grid">
                {members.map(m => (
                  <div key={m.id} className="member-chip">{m.name}</div>
                ))}
              </div>
            </div>
            <button className="enter-expense-btn" onClick={onEnterExpense}>
              + Enter Expense
            </button>
          </div>
        ) : (
          <div className="no-trip-placeholder">
            <p>No trips yet.</p>
            <p>Tap + in the top right to create your first trip.</p>
          </div>
        )}
      </div>
    </div>
  )
}

// ───────────────────────────────────────────────────────────────
// MAIN APP
// ───────────────────────────────────────────────────────────────
export default function App() {
  const [trips,           setTrips]           = useState([])
  const [currentTripId,   setCurrentTripId]   = useState(null)
  const [members,         setMembers]         = useState([])
  const [expenses,        setExpenses]        = useState([])
  const [loading,         setLoading]         = useState(true)
  const [activeTab,       setActiveTab]       = useState('home')
  const [editingExpense,  setEditingExpense]  = useState(null)
  const [showExpenseForm, setShowExpenseForm] = useState(false)
  const [showNewTrip,     setShowNewTrip]     = useState(false)
  const [showEditMembers, setShowEditMembers] = useState(false)

  const currentTrip = trips.find(t => t.id === currentTripId) || null

  async function loadTrips() {
    const { data } = await supabase.from('trips').select('*')
      .order('trip_date', { ascending: false, nullsFirst: false })
    if (data?.length) {
      setTrips(data)
      setCurrentTripId(prev => (prev && data.find(t => t.id === prev)) ? prev : data[0].id)
    } else {
      setTrips([]); setCurrentTripId(null)
    }
  }

  const loadMembers = useCallback(async (tripId) => {
    if (!tripId) { setMembers([]); return }
    const { data } = await supabase.from('trip_members').select('*')
      .eq('trip_id', tripId).order('id')
    setMembers(data || [])
  }, [])

  const loadExpenses = useCallback(async (tripId) => {
    if (!tripId) { setExpenses([]); return }
    const { data } = await supabase.from('expenses').select('*, expense_participants(*)')
      .eq('trip_id', tripId).order('expense_date', { ascending: false })
    setExpenses(data || [])
  }, [])

  useEffect(() => { loadTrips().finally(() => setLoading(false)) }, [])
  useEffect(() => {
    if (currentTripId) { loadMembers(currentTripId); loadExpenses(currentTripId) }
  }, [currentTripId, loadMembers, loadExpenses])

  function handleSelectTrip(id) { setCurrentTripId(id); setActiveTab('home') }

  function handleNewTripSaved(newId) {
    setShowNewTrip(false); setCurrentTripId(newId)
    loadTrips(); loadMembers(newId); loadExpenses(newId)
  }

  async function handleDeleteExpense(id) {
    if (!window.confirm('Delete this expense?')) return
    await supabase.from('expense_participants').delete().eq('expense_id', id)
    await supabase.from('expenses').delete().eq('id', id)
    loadExpenses(currentTripId)
  }

  function handleExpenseSaved() {
    setShowExpenseForm(false); setEditingExpense(null)
    loadExpenses(currentTripId); setActiveTab('expenses')
  }

  if (loading) return (
    <div className="loading-screen">
      <div className="loading-spinner" />
      <span>Loading RyanTravel...</span>
    </div>
  )

  const titles = { home: currentTrip?.name || 'RyanTravel', expenses: 'Expenses', summary: 'Summary' }

  return (
    <div className="app-shell">
      <div className="app-header">
        <h1>{titles[activeTab]}</h1>
        {activeTab === 'home' && (
          <button className="header-btn" onClick={() => setShowNewTrip(true)}>+</button>
        )}
      </div>

      {showExpenseForm ? (
        <ExpenseFormScreen
          trip={currentTrip} members={members} expense={editingExpense}
          onSave={handleExpenseSaved}
          onCancel={() => { setShowExpenseForm(false); setEditingExpense(null) }}
        />
      ) : activeTab === 'home' ? (
        <HomeScreen
          trips={trips} currentTrip={currentTrip} members={members} expenses={expenses}
          onSelectTrip={handleSelectTrip}
          onEnterExpense={() => { setEditingExpense(null); setShowExpenseForm(true) }}
          onEditMembers={() => setShowEditMembers(true)}
        />
      ) : activeTab === 'expenses' ? (
        <ExpensesScreen expenses={expenses}
          onEdit={e => { setEditingExpense(e); setShowExpenseForm(true) }}
          onDelete={handleDeleteExpense}
        />
      ) : (
        <SummaryScreen trip={currentTrip} expenses={expenses} members={members} />
      )}

      {!showExpenseForm && (
        <nav className="bottom-nav">
          {[['home','🏠','Home'],['expenses','💰','Expenses'],['summary','📊','Summary']].map(([tab,icon,label])=>(
            <button key={tab} className={activeTab===tab?'active':''} onClick={()=>setActiveTab(tab)}>
              <span className="nav-icon">{icon}</span>{label}
            </button>
          ))}
        </nav>
      )}

      {showNewTrip && (
        <TripFormModal onSave={handleNewTripSaved} onClose={() => setShowNewTrip(false)} />
      )}
      {showEditMembers && currentTrip && (
        <EditMembersModal tripId={currentTripId} members={members}
          onSave={() => { setShowEditMembers(false); loadMembers(currentTripId) }}
          onClose={() => setShowEditMembers(false)}
        />
      )}
    </div>
  )
}
